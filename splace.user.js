...
// @version      6.0
// @description  Left-to-right line renderer with stable queue, smart stats, mobile touch support, manual coordinates and minimizable logo GUI
// @author       ChatGPT & User
...
(function () {
    ...
    const PALETTE = [
        ...
        [31,248,178,119],[32,170,170,170],[33,165,14,30],[34,250,128,114],[35,228,92,26],
        [36,214,181,148],[37,156,132,49],[38,197,173,49],[39,232,212,95],[40,74,107,58],
    ...
    function createUI() {
        ...
        let hasMoved = false; // Отличает перетаскивание от клика разворачивания
        ...
        function dragMove(e) {
            ...
            if (e.type === 'touchmove') e.preventDefault(); // Запрет скролла экрана при драге на телефонах
            ...
            // Если сдвиг больше 4px, считаем это полноценным перетаскиванием
        ...
        function dragEnd() {
            if (isDragging) {
                ...
                // Если бот был свернут, и мы просто тапнули (не перетаскивали) — разворачиваем его
        ...
        // Слушатели ручного ввода координат
    ...
    function startPrintingLoop(customX, customY) {
        ...
        async function drawNext() {
            ...
            // Вывод текущего прогресса в статус
        ...
        try {
            ...
            if (
                ...
                if (tile && tile.pixels && tile.pixels.x) {
                    ...
                    // Авто-старт при первом перехвате, если картинка в очереди
